/**
 * Design Palette - Single Source of Truth
 *
 * Framework-agnostic color palette using RGB space format (space-separated values).
 * This format is compatible with NativeWind CSS variables and can be converted to
 * hex or other formats as needed.
 *
 * Format: 'R G B' (e.g., '14 165 233' for Electric Blue)
 */

// Electric Blue - Primary Action Color
const primary = {
  0: '224 248 255',
  50: '186 230 253',
  100: '125 211 252',
  200: '56 189 248',
  300: '14 165 233',
  400: '14 165 233',
  500: '14 165 233',
  600: '2 132 199',
  700: '3 105 161',
  800: '7 89 133',
  900: '12 74 110',
  950: '8 47 73',
};

// Deep Navy - Dark Backgrounds
const secondary = {
  0: '240 241 245',
  50: '226 228 235',
  100: '199 202 214',
  200: '147 153 176',
  300: '95 104 138',
  400: '43 55 100',
  500: '26 31 54',
  600: '26 31 54',
  700: '22 26 45',
  800: '18 22 37',
  900: '14 17 31',
  950: '10 14 39',
};

// Brand Gradient (Orange to Red)
const tertiary = {
  0: '255 244 241',
  50: '255 230 224',
  100: '255 207 196',
  200: '255 170 155',
  300: '255 133 114',
  400: '255 107 53',
  500: '255 107 53',
  600: '255 68 42',
  700: '255 38 26',
  800: '255 23 68',
  900: '230 21 61',
  950: '204 18 54',
};

// Error - Red
const error = {
  0: '255 244 241',
  50: '255 230 224',
  100: '255 207 196',
  200: '255 170 155',
  300: '255 133 114',
  400: '255 68 68',
  500: '255 23 68',
  600: '230 21 61',
  700: '204 18 54',
  800: '179 16 47',
  900: '153 14 40',
  950: '128 11 34',
};

// Success - Green
const success = {
  0: '236 253 245',
  50: '209 250 229',
  100: '167 243 208',
  200: '110 231 183',
  300: '52 211 153',
  400: '16 185 129',
  500: '16 185 129',
  600: '5 150 105',
  700: '4 120 87',
  800: '6 95 70',
  900: '6 78 59',
  950: '2 44 34',
};

// Warning - Yellow
const warning = {
  0: '254 252 232',
  50: '254 249 195',
  100: '254 240 138',
  200: '253 224 71',
  300: '250 204 21',
  400: '251 191 36',
  500: '251 191 36',
  600: '245 158 11',
  700: '217 119 6',
  800: '161 98 7',
  900: '133 77 14',
  950: '113 63 18',
};

// Info - Electric Blue (same as primary)
const info = {
  0: '224 248 255',
  50: '186 230 253',
  100: '125 211 252',
  200: '56 189 248',
  300: '14 165 233',
  400: '14 165 233',
  500: '14 165 233',
  600: '2 132 199',
  700: '3 105 161',
  800: '7 89 133',
  900: '12 74 110',
  950: '8 47 73',
};

// Typography - Soft White to Medium Gray
const typography = {
  0: '255 255 255',
  50: '248 250 252',
  100: '241 245 249',
  200: '226 232 240',
  300: '203 213 225',
  400: '148 163 184',
  500: '100 116 139',
  600: '71 85 105',
  700: '51 65 85',
  800: '30 41 59',
  900: '15 23 42',
  950: '2 6 23',
};

// Outline - Light Gray
const outline = {
  0: '255 255 255',
  50: '248 250 252',
  100: '241 245 249',
  200: '226 232 240',
  300: '203 213 225',
  400: '148 163 184',
  500: '100 116 139',
  600: '71 85 105',
  700: '51 65 85',
  800: '30 41 59',
  900: '15 23 42',
  950: '10 14 39',
};

// Background - Deep Navy Base
const background = {
  0: '248 250 252',
  50: '226 232 240',
  100: '203 213 225',
  200: '148 163 184',
  300: '100 116 139',
  400: '71 85 105',
  500: '51 65 85',
  600: '45 49 66',
  700: '38 42 57',
  800: '26 31 54',
  900: '18 22 37',
  950: '10 14 39',
};

// Background Special (semantic colors)
const backgroundSpecial = {
  error: '255 244 241',
  warning: '254 252 232',
  success: '236 253 245',
  muted: '26 31 54',
  info: '224 248 255',
};

// Focus Ring Indicators
const indicator = {
  primary: '14 165 233',
  info: '14 165 233',
  error: '255 23 68',
};

// Gradient Tokens
const gradient = {
  // Smooth gradient: light blue → aqua → light green
  brandFrom: '186 230 253', // Light blue (primary[50])
  brandVia: '209 250 229', // Light mint green (success[50])
  brandTo: '236 253 245', // Very light green (success[0])
};

// Spacing scale (in pixels, aligned with Tailwind/Gluestack)
const spacing = {
  px: '1px',
  0: 0,
  1: 4,
  2: 8,
  3: 12,
  4: 16,
  5: 20,
  6: 24,
  8: 32,
  10: 40,
  12: 48,
  16: 64,
  20: 80,
  24: 96,
  32: 128,
};

// Border radii scale (in pixels)
const radii = {
  none: 0,
  xs: 2,
  sm: 4,
  md: 6,
  lg: 8,
  xl: 12,
  '2xl': 16,
  '3xl': 24,
  full: 9999,
};

/**
 * Complete design palette export
 */
module.exports = {
  primary,
  secondary,
  tertiary,
  error,
  success,
  warning,
  info,
  typography,
  outline,
  background,
  backgroundSpecial,
  indicator,
  gradient,
  spacing,
  radii,
};
